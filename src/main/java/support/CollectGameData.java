...
public class CollectGameData {
  ...
  private static GameState collectGameData(int gameID,
    ...
    while (rowSet.next()) {
      gamePlayers.add(new Player(rowSet));
    }
    ...
    while (rowSet.next()) {
      Focus working = new Focus(rowSet);
      gameFocuses.put(working.round, working);
    }

    // Get Legacies
    rowSet = DatabaseReads.getLegacies(gameID);
    while (rowSet.next()) {
      Legacy working = new Legacy(rowSet);
    ...
    while (rowSet.next()) {
      Character working = new Character(rowSet);
      gameCharacters.put(working.id, working);
    }
