...
public final class SetScenePlayerCharacters extends HttpServlet {

  private static final long serialVersionUID = 1L;

  @Inject
  public SetScenePlayerCharacters() {}
    ...
    if (!Settings.DEBUG) {
      ...
      if (CPA.getPlayer() != userID) {
        ...
        return;
          ...
          && CPA.getAction() != ActiveGameState.PLAY_LEGACY_ES) {
        System.err.println("It is not time to set the characters in a scene");
        response.setContentType("application/json");
        PrintWriter writer = response.getWriter();
        writer.print("{\"success\" : false}");
        writer.flush();
